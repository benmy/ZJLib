...
- (void)showIndicatorWithTitle:(NSString*)title{
    ...
    indicator.center = CGPointMake(self.bounds.size.width - indicator.frame.size.width, self.bounds.size.height / 2);
...
//    objc_setAssociatedObject(self, &kButtonTextObjectKey, currentButtonText, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    ...
    [self setTitleEdgeInsets:UIEdgeInsetsMake(0, -indicator.frame.size.width, 0, 0)];
    self.enabled = NO;
